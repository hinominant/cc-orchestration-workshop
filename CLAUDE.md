...
- **対象**: 非エンジニア（コード未経験OK）+ エンジニア（Claude Code未経験OK）
...
Claude Code未経験のエンジニアがAIエージェントチームを設計・運用するスキルを習得する。

| STEP | 内容 | 時間 |
|------|------|------|
| 0 | Claude Codeとは（全体像・アーキテクチャ） | 15分 |
| 1 | 基本操作とCLAUDE.md（CLI・スラッシュコマンド・設計思想） | 20分 |
| 2 | パーミッションとセキュリティ（サンドボックス・Hooks・組織ポリシー） | 25分 |
| 3 | エージェント定義の設計（ファイル構造・役割分担） | 30分 |
| 4 | オーケストレーション構成（Hub-spoke・並列実行） | 30分 |
| 5 | 実践: バックエンドサービス構築 | 60分 |
| 6 | テスト・品質管理（QAスコアリング・外部監査） | 30分 |
| 7 | チーム運用とスケーリング（導入パターン・ベストプラクティス） | 30分 |

#### エンジニア編の特徴

- **対象**: エンジニア（ターミナル操作可能、Claude Code未経験OK）
- **深掘り**: セキュリティ設計、エージェント定義の書き方、並列実行パターン
- **実践**: バックエンドサービス（EOS）をエージェントチームで構築
- **スターターリポジトリ**: `hinominant/cc-workshop-intermediate-starter`（方法Aの場合）

### 事前準備ページ

全コース共通の事前準備情報を `/preparation` に集約。
- Claude Codeの概要説明
- Mac / Windows セットアップ手順
- 事前に読むべき記事（公式ドキュメント、セキュリティ設定ガイド等）
- セキュリティ設定ガイド（10項目）
- 準備物チェックリスト
